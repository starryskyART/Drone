...
/**
  ...
  * @param {IMU_Data} *gyroAccel mpu的6轴数据
  * @param {Euler_struct} *eulerAngle 计算后得到的欧拉角
  * @param {float} dt 采样周期 (单位s)
  * @return {*}
  */
void Common_IMU_GetEulerAngle(IMU_Data *gyroAccel, Euler_struct *eulerAngle,float dt)
{
      volatile struct V
      {
            float x;
